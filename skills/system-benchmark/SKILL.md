...
What mature systems look like: WCAG 2.2 AA conformance verified by audit. (Some legal baselines, e.g. EN 301 549, still reference WCAG 2.1 AA; use that if it's the team's obligation.) Automated accessibility checks in CI plus regular manual testing with screen readers. Reduced motion and high contrast support built in.
...
What mature systems look like: Most eligible products are on the system. Off-system builds are rare and tracked. The system is the default. (If you cite an adoption figure for a named system, it needs a source.)
...
What mature systems look like: CLI scaffold, framework templates, IDE plugins, hot-reload dev environment. A new engineer renders a first component within their first session — measure time-to-first-component for the user's system rather than quoting a target.
...
**Government**: US Web Design System (USWDS), Australian Government Design System (AGDS), GOV.UK Design System, GC Design System (Government of Canada)

**Product**: Primer (GitHub), Spectrum (Adobe), Paste (Twilio), Base Web (Uber), Orbit (Kiwi.com), Garden (Zendesk)

**Agency**: Few agencies publish their systems. For an agency system, compare against product systems of similar scale and say that's what you did.
...
- Estimated maturity stage (Ad-hoc / Managed / Systematic / Measured / Optimised)
- Notable strengths and weaknesses

Every claim about a comparison target carries a source: the docs site or repository URL you read it from, with the date you checked. If you can't browse, label the claim "from training knowledge, unverified, as of [knowledge date]" — component counts, token approaches and governance models change between releases. Never present an unsourced count or maturity stage as fact, and never fabricate data about comparison targets.
...
**Maturity stage:** [Ad-hoc / Managed / Systematic / Measured / Optimised] — closest profile from Step 2: [e.g. Established enterprise]
...
|---|---|---|
| Foundation quality | 🟢 Strong / 🟡 Functional / 🟠 Weak / 🔴 Absent | [One sentence] |
...
**[Dimension name]:** 🟢 Strong / 🟡 Functional / 🟠 Weak / 🔴 Absent

Evidence: [What was observed]
Gap: [What specifically needs to happen to reach the next status label]
Comparison: [How this compares to similar systems — "Material Design has X at this layer, which your system lacks" — with the source for each claim about the other system]
...
|---|---|---|---|---|
| Token architecture | [Status] | [What they have + source] | [What they have + source] | [What they have + source] |
...
- What the next status label looks like for this dimension
...
**Current state:** [Maturity stage] — [key characteristics]
...
**Scope**
- **Inspected:** [the user's files, directories, and data sources actually read; comparison-target sources consulted, with URLs]
- **Not inspected:** [what was out of reach — e.g. comparison targets' internal governance, which isn't public]
...
- **Assumptions:** [anything taken as given, including every comparison claim labelled "from training knowledge, unverified"]
...
- Compare dimension statuses — which improved, which held steady, which regressed
- Name the trend section "Benchmark trend", showing movement per dimension and highlighting any dimension that changed status
- Update the maturity roadmap targets based on actual progress
...
- No data about comparison targets is fabricated; every claim about a target cites a URL or is labelled "from training knowledge, unverified, as of [date]"
- The improvement roadmap is specific (not "improve tokens" but "add semantic tier aliases for feedback and status categories")
- The Scope block states the limitations: comparison target assessments are based on public information, not audited
