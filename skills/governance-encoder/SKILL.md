...
description: "Turn written design system governance into machine-checkable JSON rules in .ai/governance/ for agents, linters and CI. Triggers: governance as code, encode rules, rule engine, enforce automatically. Explaining why a decision was made: decision-record. CI pipeline files: cicd-integration."
references:
...
- **Encoding** says: `{ "rule": "component_naming", "pattern": "^[A-Z][A-Za-z0-9]*$", "scope": "component_export_name", "severity": "error", "message": "Component names must be PascalCase with no special characters" }`
...
- `integrations.*` — existing governance context (see below)
...
**Optional fields:**
- `status` — `active` for rules found in an inspected source or confirmed by the user (default); `proposed` for suggestions the team hasn't adopted
...
```json
{ "type": "pattern", "pattern": "^[A-Z][A-Za-z0-9]*$", "target": "export_name" }
```
...
Encode only rules you found in an inspected source (with that source in `references.governance_doc`) or that the user confirmed. The category lists below are prompts for where to look and what to ask about, not rules to emit. If a category has an obvious gap, write the suggestion to a separate `rules/proposed.json` with `"status": "proposed"` so it can't be mistaken for the team's actual policy, and list it for the user to accept or drop.
...
- Semver policy (encode the team's stated policy; `version-bump-advisor` owns the major/minor/patch classification)
...
.ai/governance/
  rules/
    ...
    release.json
    documentation.json
    proposed.json        # generic suggestions, "status": "proposed"
  exceptions/
...
Compute every count from the rule files you wrote; the placeholders below show the shape only.
...
```json
{
  ...
  "sources_inspected": ["[file or doc path]"],
  "active_exceptions": "[n]"
}
```
...
- Every `active` rule traces to an inspected source or user confirmation; generic suggestions are in `proposed.json` only
- The output ends with a Scope note: sources inspected, sources not inspected (wikis, Slack, team knowledge you couldn't reach), and active vs proposed rule counts. Don't claim the encoded set is complete
