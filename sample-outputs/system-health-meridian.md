...
**Source:** Meridian Design System — React/Tailwind component library
...
Meridian is internally consistent and well documented, but adoption is uneven across product teams and governance lives in people's heads rather than on paper. Those two dimensions are what's holding it back. Here's the dimension-by-dimension picture.

## Overall health

| Dimension | Status | Key finding |
|---|---|---|
| Tokens | 🟢 Strong | Complete tier structure with no leakage; primitive tokens lack descriptions |
| Components | 🟡 Functional | Consistent APIs; error states undocumented and compound coverage thin |
| Documentation | 🟢 Strong | 28 of 30 components fully documented; two recent additions pending |
| Adoption | 🟠 Weak | Uneven across teams (under 5% to 70%+) and not tracked |
| Governance | 🟠 Weak | Contribution criteria and deprecation process are informal |
| AI readiness | 🟢 Strong | Consistent six-section descriptions; no component manifest yet |
| Platform maturity | 🟠 Weak | No semantic versioning, irregular releases, no migration guides |

Status key: 🟢 Strong · 🟡 Functional · 🟠 Weak · 🔴 Absent

**Maturity stage (inferred from evidence):** Managed — the library exists with established tokens and consistent documentation, but contribution, deprecation and adoption tracking are informal. Adoption and governance are the dimensions stopping it reaching Systematic.

---

## Summary

The Meridian system is at a critical junction. Token and documentation work is strong — the foundation is solid. AI readiness is surprisingly mature for a Managed system. But adoption is the constraint: teams are not using what has been built. Governance is informal, which means decisions are not recorded and processes are opaque to consuming teams. The next six months should focus on raising adoption and documenting governance.

---

## Maturity stage

**Why Managed, not Systematic:** The missing elements are explicit governance processes and adoption tracking. Systematic requires documented contribution criteria, a deprecation process, and adoption signals that inform prioritisation. Meridian has informal versions of all three — the design systems team knows who should contribute and when a component should be deprecated, but these processes are not documented. A new team member cannot predict how to propose a component. Consuming teams cannot see whether their feedback about missing components is considered. This informality limits adoption and creates support burden.

**Path to Systematic:** Document the contribution and deprecation processes explicitly (4 weeks). Establish recurring adoption tracking with a simple dashboard showing component usage by team (2 weeks). Run a governance communication sprint: announce the processes to all consuming teams and record the key decision rationale for existing components (2 weeks). With these complete, the system meets the Systematic criteria.
...
Both are recent additions (added in the last four weeks). They are building-blocked: Storybook is running but documented usage guidelines have not been written. This is a known gap and the documentation is scheduled for completion by end of week. No action needed; noting the recency to calibrate the status.
...
Three teams (Workspace, Directory, Finance) have 70%+ adoption of system components. Two teams (Mobile, Analytics) have 20–30% adoption. One team (Integrations) has <5% adoption and builds almost everything locally. The system is optional, not required, which means adoption depends on team choice and incentives. No metric is tracked that would help understand why adoption varies (is it feature gaps, documentation, team preference, or something else?).
...
**Scope**
- **Inspected:** Meridian's Figma library, the published npm package, and the GitHub repository (token files, component source, Storybook stories and docs)
- **Not inspected:** consuming teams' product repositories, support-channel history, and any documentation outside Storybook and the repository
- **How "none found" was checked:** the missing error-state documentation was checked against every Input, Select and TextArea story and docs page, the same set where disabled-state docs were found
- **Assumptions:** per-team adoption figures and the contribution criteria were reported by the design systems team, not measured

> **A note on context:** This assessment sees your system's artefacts — it does not see the history, constraints, or trade-offs behind them. If any finding describes an intentional decision or a known limitation, let me know and I'll calibrate future assessments to your system's actual priorities.

---

*Generated by Design Systems OS — `skills/system-health` skill*
*Source repository: Meridian Design System (React 19, Tailwind v4, Figma + Storybook)*
