...
**Source:** A product design system (name withheld) — React component library with CSS Modules
...
the system is a mature, well-maintained component library with strong fundamentals. The library has an appropriate complexity distribution (foundation-heavy, compound-moderate, feature-light), good API consistency, and comprehensive documentation. The most significant finding is coverage of data-visualization components — the system has none, yet data visualization is a core need across the product domain products. Recommendations focus on closing this gap and maintaining the strong standards the library has established.
...
**F1-1: Strong adoption across primary products.** npm downloads show 8,500–9,200 downloads per week (last 90 days), with a growth trend of +3% week-over-week. This is consistent with the growth of the products that consume the system. Code search across 12 consuming repositories identified 891 distinct import statements across 44 of 47 components (usage in actual product code, not just installed).
...
**Feature components (6 total):** SummaryCard (domain-specific), ProfileCard (domain-specific), ActivityRow (domain-specific), StatusBadge, HealthIndicator, AuditLog. These are higher-specific components that solve problems particular to the product domain. All are actively used and well-adopted.
...
**F2-2: Feature components are domain-appropriate but at risk of over-specialization.** All 6 feature components are domain-specific. None would be useful in a other product. This is appropriate — the system is designed for the product domain products. However, the team should monitor whether feature components accumulate at a rate faster than they are used. (Current ratio: 6 feature components, all used in 2+ products — healthy.)
...
| Card vs. SummaryCard | SummaryCard extends Card with domain-specific styling (pink accent border, custom badge positioning). Could be a variant of Card instead of a distinct component. | Likely redundant — SummaryCard is a styled specialization, not a distinct pattern. | Assess whether SummaryCard can become `Card variant="portfolio"` or whether the styling difference is significant enough to justify a separate component. If it can be a variant: 2 hours refactoring. |
...
**F3-3: SummaryCard may be over-specialized.** If it is purely a Card with custom styling, it should be a variant. If it has different compositional rules or accessibility contracts, it justifies being separate.
...
Surveyed 6 consuming product teams (dashboard, analytics, reporting, admin, support, mobile). Asked: "What interface patterns do you regularly build that the system does not provide?" Correlated with code search for third-party library imports that suggest workarounds.

### Identified gaps

| Gap | Evidence | Urgency | Recommendation |
|---|---|---|---|
| **Data visualization (chart, graph, heatmap)** | 5 of 6 teams report need. Code search: 4 instances of Chart.js imports, 3 of Recharts, 2 of D3. Not for rendering libraries, but for chart wrapper components that provide the system theming. | Critical — multiple products need it, all building locally. | Propose as Q2 contribution: build 3–4 basic chart components (Line, Bar, Pie, Heatmap) wrapping Recharts. Effort: 24 hours (design + implementation + docs). Blocks: stakeholder alignment on which charting library to standardize on. |
| **DataTable advanced features** | The system's DataTable supports sorting, pagination. Three teams need column resizing, freezing, and export-to-CSV. Currently building these features locally or using third-party table libraries. | High — tight coupling to product roadmap (trading product needs column resize this quarter). | Extend DataTable with optional props for column resizing (`resizable: boolean`) and freezing (`frozenColumns: number`). Effort: 12 hours. Timeline: 2 sprints (design, spec, build, test). |
...
| **Time and date range picker** | the system DatePicker is single-date. Two teams need date ranges (start date + end date). | Medium — used in specific products (analytics dashboards) but not universal. | Build DateRangePicker as an extension of DatePicker. Effort: 16 hours. Can be planned after DataTable enhancements. |
...
**Assessment:** the system has good coverage of the 80% use case. The 20% gaps are real but appropriately scoped for either contribution (data viz) or incremental enhancement (DataTable).
...
| **Card** | 7 | DataTable, Modal (content wrapper), ProfileCard, SummaryCard | 0 | Compound |
...
**TimelineEvent** (0 fan-in, 0 fan-out) — no dependencies either way, so deprecation has no in-library blast radius. The case for deprecating it comes from the usage evidence in F1-2, not from being standalone.

**LegendItem** (0 fan-in, 0 fan-out) — same position. Its deprecation case rests on F1-2 and the roadmap question, not on the graph.
...
### Per-component checklist (sample)

| Component | Purpose clarity | Prop docs | Anti-patterns | Composition | Accessibility | Status |
|---|---|---|---|---|---|---|
| Button | ✅ | ✅ | ✅ | ✅ | ✅ | 🟢 Strong |
| Input | ✅ | ✅ | ✅ | ⚠️ | ✅ | 🟢 Strong |
| Modal | ✅ | ✅ | ⚠️ | ✅ | ✅ | 🟢 Strong |
| DataTable | ✅ | ✅ | ⚠️ | ⚠️ | ⚠️ | 🟡 Functional |
| TimelineEvent | ⚠️ | ✅ | ❌ | ⚠️ | ⚠️ | 🟠 Weak (low usage) |
| Card | ✅ | ✅ | ⚠️ | ✅ | ✅ | 🟢 Strong |
| Tooltip | ✅ | ⚠️ | ⚠️ | ⚠️ | ⚠️ | 🟡 Functional |
| Flex | ⚠️ | ✅ | ❌ | ✅ | ❌ | 🟠 Weak (utility, limited context) |

✅ PASS = documented in full · ⚠️ WARN = partial · ❌ FAIL = missing. 6 of the 8 sampled components have no ❌ FAIL.

### System-level indicators

| Indicator | Status | Assessment |
|---|---|---|
| Component manifest exists | ✓ Yes, JSON at `packages/ui/manifest.json` | 🟢 Strong — current and maintained |
| Structured metadata | 45/47 (96%) have machine-readable metadata | 🟢 Strong |
| Description consistency | 46/47 (98%) follow six-section format | 🟢 Strong |
| Token documentation | 34/42 tokens (81%) have intent descriptions | 🟡 Functional — minor gaps |
| Figma-code sync | 44/47 (94%) — 3 components have outdated Figma descriptions | 🟡 Functional — minor maintenance needed |

### Findings

**AI-R1: System-level AI readiness is strong.** 6 of the 8 sampled components have no failing checklist item. The manifest exists and is current. Description format is consistent across the library.

**AI-R2: Weak components are concentrated in low-usage or utility components.** TimelineEvent (one ❌), Flex (two ❌), and Tooltip (four ⚠️) are all low-priority for AI readiness work because they are either unused or utilities. Focusing AI-readiness improvement efforts on high-fan-in components (Button, Input, Card) has minimal ROI because these already pass on four or five of the five items.
...
**Assessment:** the system is AI-ready. An LLM reading the component manifest and descriptions can reliably select and configure components without human intervention.
...
5. **Assess SummaryCard specialization.** Determine whether SummaryCard can become a Card variant or justifies being separate. If variant: refactor (2 hours). If separate: document the distinction (1 hour).
...
**Scope**
- **Inspected:** the system source in `packages/ui/` (47 components), the component manifest, npm download stats (last 90 days), Storybook visit analytics, and GitHub code search across 12 consuming repositories
- **Not inspected:** consuming repositories outside those 12, Figma library analytics (detach rates), support-channel history
- **How "none found" was checked:** the zero-import result for TimelineEvent and LegendItem used the same import search that found 891 imports of the other 44 components, including aliased imports, across all 12 consuming repos
- **Assumptions:** the 12 repositories are all of the system's consumers; npm and Storybook figures reflect product usage rather than CI installs
...
- ✓ AI-readiness checklist is component-specific and actionable
- ✓ Action list prioritises by impact, not ease

---

*Generated by Design Systems OS — `skills/component-audit` skill*
*Source repository: name withheld (React, CSS Modules)*
