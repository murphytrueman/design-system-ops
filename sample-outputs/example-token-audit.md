...
**Source:** Product design system (name withheld) — CSS custom properties + JSON tokens (42 files, ~480 tokens)
...
|---|---|---|---|
| TA-01 | 🔴 Critical | Value | **9 component tokens reference primitives directly, bypassing the semantic tier.** `--ds-card-border` resolves to `var(--ds-gray-200)` instead of routing through `var(--ds-border-default)`. Same pattern in toast, dialog, and tooltip border tokens. This means theme switching breaks these components — the primitive value does not change between themes, but the semantic token does. |
| TA-02 | 🔴 Critical | Value | **Feedback colour tokens are disconnected from the feedback system.** `--ds-toast-success-bg` hardcodes `#DCFCE7` instead of referencing `var(--ds-feedback-success-subtle)`. The semantic layer already defines these roles correctly — the component tokens just do not use them. 4 toast variants and 3 badge variants affected. |
| TA-03 | 🟠 High | Naming | **Semantic colour tokens mix intent-based and appearance-based names.** `--ds-action-primary` (intent) coexists with `--ds-blue-surface` (appearance). The blue-surface token should be `--ds-surface-accent` or `--ds-surface-brand` — describing what it is for, not what colour it is. 8 tokens use colour-based names at the semantic tier. |
| TA-04 | 🟠 High | Coverage | **No semantic tokens exist for interactive states.** Hover, focus, active, and disabled colours are defined at the component tier but skip the semantic tier entirely. `--ds-button-hover-bg` resolves directly to a primitive (`--ds-blue-600`). This forces every new component to reinvent state colours instead of referencing a shared semantic set. |
| TA-05 | 🟠 High | Naming | **Spacing tokens use `px` suffix in names.** `--ds-space-4px`, `--ds-space-8px`, `--ds-space-16px` — the suffix duplicates what the value already expresses. If the base unit ever changes (e.g., moving to rem), the names become lies. Standard practice: `--ds-space-1`, `--ds-space-2`, `--ds-space-4` (multiplier-based) or `--ds-space-xs`, `--ds-space-sm`, `--ds-space-md` (T-shirt). |
| TA-06 | 🟡 Medium | Value | **3 alias chains are 4 hops deep.** `--ds-dialog-overlay-bg` → `--ds-surface-overlay` → `--ds-neutral-overlay` → `--ds-black` → `#000000`. Each hop adds fragility and makes debugging harder. 3 hops is the practical maximum — compress to 3 by removing the redundant `--ds-neutral-overlay` layer. |
| TA-07 | 🟡 Medium | Coverage | **Typography semantic tokens exist for size and weight but not for line-height or letter-spacing.** Components set `line-height: 1.5` and `letter-spacing: -0.01em` as raw values. These should be tokenised at the semantic tier (`--ds-leading-normal`, `--ds-tracking-tight`) so typography can be adjusted systematically. |
| TA-08 | 🟡 Medium | Naming | **Component token prefix is inconsistent.** Button tokens use `--ds-button-*`, card tokens use `--ds-card-*`, but toast tokens use `--ds-cmp-toast-*` and dialog uses `--ds-ui-dialog-*`. Pick one convention and migrate. The most common pattern (used by 7 of 10 component token files) is `--ds-{component}-*`. |
| TA-09 | ⚪ Low | Coverage | **No token exists for focus ring offset.** Components use `outline-offset: 2px` as a raw value in 14 places. Create `--ds-focus-ring-offset` as a primitive and reference it from a semantic `--ds-focus-offset` token. |
| TA-10 | ⚪ Low | Naming | **Z-index tokens use raw values as names.** `--ds-z-100`, `--ds-z-500`, `--ds-z-1000` — workable but brittle if a new layer needs to be inserted between existing values. Consider named layers: `--ds-z-dropdown`, `--ds-z-modal`, `--ds-z-toast`. |
| TA-11 | ⚪ Low | Value | **2 orphaned primitive tokens are defined but never referenced.** `--ds-purple-50` and `--ds-purple-100` exist in the colour primitive file but are not referenced by any semantic or component token. If these colours are unused, remove them to keep the primitive tier clean. |

### DTCG 2025.10 compatibility assessment

The The system token system uses CSS custom properties as its primary format, with a JSON source-of-truth file that generates the CSS output via a build script.

**Current format:**
```css
:root {
  --ds-blue-500: #2563EB;
  --ds-action-primary: var(--ds-blue-500);
  --ds-button-bg: var(--ds-action-primary);
}
```
...
7. TA-08: Standardise component token prefix to `--ds-{component}-*`
...
*Source: Product design system, name withheld (CSS custom properties, JSON source, React 18, TypeScript, Tailwind v3)*
