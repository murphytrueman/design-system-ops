...
class TestBundleBuild(unittest.TestCase):
    ...
    def test_shipped_bundle_passes_its_own_install_check(self):
        """The as-shipped check: unpack the bundle the way an installer would
        and run the doctor script that ships inside it. Every other test here
        reads the repo; this one reads what users actually download."""
        unpacked = tempfile.mkdtemp(prefix="dsops-shipped-")
        try:
            with zipfile.ZipFile(self.zip_path) as bundle:
                bundle.extractall(unpacked)
            result = subprocess.run(
                ["bash", os.path.join(unpacked, "verify-install.sh")],
                stdout=subprocess.PIPE,
