...
Each template shows the minimum viable structure. Example values in the templates are illustrative. In real output, each rule carries a `source:` (file path, URL, or `user`); a rule with no source carries `status: proposed`. Real output will contain more entries per section, populated from the target design system's actual components, tokens, patterns, and rules.
...
context_engine:
  ...
  blueprints:
    - id: ux
      ...
      coverage: "[n of N patterns documented, e.g. 12 of 40]"
    - id: ui
      file: ui-blueprint.yml
      coverage: "[n of N semantic tokens with intent]"
    - id: content
      file: content-blueprint.yml
      coverage: "[n of N content patterns documented]"
    - id: accessibility
      file: accessibility-blueprint.yml
      coverage: "[n of N components with a11y contracts]"
    - id: ethical
      file: ethical-blueprint.yml
      coverage: "[n sourced rules, n proposed]"
    - id: technical
      file: technical-blueprint.yml
      coverage: "[n of N component APIs documented]"
    - id: business_intelligence
      file: business-intelligence-blueprint.yml
      coverage: "[n of N components with sourced outcome mapping]"
  skipped: ["[blueprint id]: [why, e.g. no source material]"]
```
