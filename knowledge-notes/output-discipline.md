...
At the end of every audit or assessment, include a brief Scope block in this shape:
...
- **How "none found" was checked:** [for any absence claim, how the search was shown to work, per the next section — omit if the report makes no absence claims]
...
## An empty result needs proof the search could succeed

Zero findings is a claim like any other. A search pattern that doesn't fit this codebase returns nothing, and nothing looks exactly like clean. Before reporting that something is absent, confirm the search could have found it: that the pattern matches at least one known example in this codebase, or that the files it ran over are the ones that would contain it.

**Wrong:** "No hardcoded colour values found."
**Right:** "No hardcoded colour values found across 84 component stylesheets. The search matches hex, `rgb()` and `hsl()` values, and does find them in the token source files, so it works on this codebase."

**Wrong:** "Every component has documentation."
**Right:** "All 42 components in `src/components/` matched a Storybook story by file path. The 3 components exported from `src/index.ts` but living elsewhere were not checked."

If you can't show the search would have found a real example, say the result is unconfirmed rather than reporting it as clean.

---

## Every figure and fact needs a source

A plausible number reads exactly like a measured one. So every figure, date, name, link, and piece of rationale in the output comes from somewhere the reader could check: a file you inspected, a tool result, or something the user told you.

- **Facts about the user's system** come from inspected files or from the user. If neither, don't state them.
- **Figures are labelled** by where they came from: measured (from a named source), estimated (with the reasoning shown), or assumed (stated as an assumption the reader can change).
- **Derived figures are recalculated** from the inputs shown, every time. Check the arithmetic before it goes in the output.
- **Numbers in a skill's examples are illustrative.** Never carry them into real output.
- **Gaps stay visible.** Where a date, owner, link, metric, or rationale is unknown, write `[needs data: what's missing]` rather than filling it in. A deliverable meant to be sent, such as an announcement, a brief, or a changelog entry, lists its open placeholders at the top. "Ready to send" means every gap is listed, not that every gap is filled.
- **Generated artefacts carry provenance.** When a skill writes documentation, metadata, rules, or blueprints, each fact traces to source, docs, or the user. Anything else is marked `proposed` or `unverified`, never presented as the system's actual behaviour or policy.

**Wrong:** "Payback period: 10 months." (from an example's numbers, not recalculated)
**Right:** "Payback lands around month 14 on these inputs: $450k a year of avoidable cost (estimated from the duplicated-work figures above), realised at 50% in year one, against $245k a year of investment (assumed; replace with your figures)."
...
- Maturity: the named stages from the component-governance note — Ad-hoc / Managed / Systematic / Measured / Optimised. Never numbered levels ("Level 3"), which read as a score
