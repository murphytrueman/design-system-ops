...
For Figma-based design systems, the [Figma Console MCP](https://github.com/southleft/figma-console-mcp) by Southleft is the recommended Layer 1 server. It provides read and write access to Figma through the Desktop Bridge plugin, including variable collections and component descriptions. The official Figma MCP server can also write to Figma (through its `use_figma` tool), but its read tools are scoped to the current selection — `get_variable_defs`, for example, returns only the variables a selected node uses, not whole collections — so collection-wide audits need the Console MCP.
...
- `figma-variable-audit` renames variables, adds modes and creates missing semantic variables directly in Figma after auditing, and owns the Figma-versus-code comparison
...
Setup requires Figma Desktop with the Desktop Bridge plugin running. The connection persists across Figma restarts. For teams that cannot run Figma Desktop (e.g. CI/CD environments), the official Figma MCP server is the fallback, with the selection-scoped limits above.

### Layer 2: The system's own knowledge (files first, a server if you build one)

This pack does not ship a design system MCP server, and most teams don't need one. Coding agents read the repository directly, and the files this pack writes are laid out for that:

- `AGENTS.md` (from `agent-instructions`) is the entry point: where things live, the rules with sources, how to check work. `CLAUDE.md`, `.cursor/rules/` and `.github/copilot-instructions.md` can point at it.
- `.ai/index/` (from `codebase-index`) holds the component inventory and dependency graph.
- `.ai/metadata/` (from `metadata-schema-generator`) holds per-component props and accessibility contracts.
- Choosing-between pages (from `component-decision-tree`) and token docs (from `token-documentation`) are linked from `AGENTS.md`.
- Enforcement lives in the lint and CI configs `governance-encoder` and `cicd-integration` write, which an agent runs rather than reads.

A team that wants the same knowledge served over MCP, to agents that don't have the repository open, can build a small server over those files. Adobe publishes `@adobe/spectrum-design-data-mcp` alongside its tokens and component API schemas, which is the shape to copy: the server serves the published design data, it doesn't invent a second copy. If you build one: version the served data with the codebase, support filtering so "all navigation components" doesn't return the whole inventory, and include the version in every response so agents can detect misalignment with Layer 1.
...
## What the pack writes for agents

Whether read from the repository or served over MCP, these are the files:

| Data source | Generated by | Location |
|---|---|---|
| Agent entry point | `agent-instructions` | `AGENTS.md` (plus pointer files on request) |
...
| Enforcement config | `governance-encoder` | ESLint, Stylelint, dependency-cruiser and CODEOWNERS files in their conventional locations, plus `docs/GOVERNANCE.md` for rules with no executable form |
| Choosing between components | `component-decision-tree` | `docs/choosing/*.md` (YAML in `.ai/decision-trees/` on request) |
| AI component descriptions | `ai-component-description` | Figma component description fields |

### If you build a server: recommended capabilities

An MCP server over these files should support these query types:
