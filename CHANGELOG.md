...
- **Four more fixture samples and five more eval cases.** The fixture gains two overlay components that do the same job with different APIs (`Modal` and `Dialog`) and a consuming app (`apps/checkout`) with a forked Button in raw values, a `--card-padding` override and a lagging dependency. New cases: `duplicate-components` (component-audit), `consumer-drift` (drift-detection), `health-baseline` (system-health), `describe-button` (ai-component-description) and `route-agent-instructions`. The runner gains a `produces` case kind for outputs that aren't finding-shaped, and grading now stops at the Scope block, because a severity marker in an Assumptions line was being read as a finding. `tests/evals/save_sample.py` turns a run into a shipped sample, trimming only the chat lead-in and any harness notes after the closing note, and says so in the header.
- **`tests/test_authoring_rules.py`** pins the rules the review applied by hand: every skill declares `allowed-tools` and loads `output-discipline`; a skill that rates severity defines all four levels; a findings template has an evidence column; no skill or note describes DTCG resolvers as having modes. `CONTRIBUTING.md` lists the rules in prose.

### Changed

- **Every shipped sample is now an unedited fixture run.** The seven hand-written or anonymised samples are gone. They contradicted the rules the skills enforce: the drift sample had no file paths and invented team intent, the component-audit sample said "surveyed 6 consuming product teams", the token-audit sample showed DTCG in the pre-2025.10 string form the skill calls a migration signal, the Carbon docs-coverage sample disagreed with its own arithmetic, the dashboard carried invented effort hours, and three still said "Design Systems OS". The nine samples that ship are the outputs of the eval cases against the fixture, regenerated from the reviewed skills, and the two that were marked "Edited: no" while carrying a chat lead-in and a note about claude.ai connectors are regenerated too.
...
- **Every skill was reviewed against the same five questions:** can it deliver what its description promises with the inputs a real user has; what does it do when the input is missing; does every finding carry evidence and a severity from a stated rubric; does it use the standards and tools a senior practitioner would expect; and does it pre-approve the commands it runs. Five changes apply to all 36 skills: an evidence column (file and line, token path, Figma node or variable id) in every findings template, and a rule that a finding without one isn't logged; a severity rubric per skill saying what earns each of the four levels, replacing bare severity keys; an explicit stop rule for missing input ("if X is absent, stop and ask", or a data-collection plan) in place of estimated tables; `allowed-tools` declared on all 36, covering every command the body runs; and `output-discipline` loaded by every skill. The per-skill changes worth knowing about:
  - **token-compliance** stops when no code is in reach, hands wrong-tier token *definitions* to token-audit, applies critical-path and fan-in severity adjustments only from stated facts (never a directory name), covers React Native and template-literal CSS, and names the Stylelint rule that stops the count growing.
  - **schema-validator** runs the repo's own tool first (Style Dictionary 4 or 5, or Terrazzo) and reconciles, validates resolver documents, checks the spec's name rules and `$deprecated`, and knows all six composite types. **theme-audit** composites alpha and converts OKLCH and P3 before computing contrast, and computes the brand-variant check it used to assert. **figma-variable-audit** reads collections in pages with the real Console MCP parameters, and reads usage data before claiming orphans. **token-documentation** writes intent back into `$description` (or the source's comment syntax) as its machine-readable output, and produces a parallel JSON only on request.
  - **component-audit** records which usage signals exist rather than asking the user to pick, names Figma library analytics as Enterprise-only, drops "Storybook visit data" (there is none), and flags complexity by count. **component-api-validator** reads `package.json` before asking, extracts props with react-docgen-typescript, vue-component-meta or the Custom Elements Manifest, and corrects its boolean advice (booleans default to `false`; `hideLabel` is fine). **design-to-code-check** names the Figma MCP tools, checks states and breakpoints from their selectors and media queries, and marks rendering "not checked" without a build. **version-bump-advisor** writes changesets or conventional-commit phrasing where the team uses them, lists the breaking changes it missed (new required prop, narrowed values, changed callback signature, removed `exports` subpath, DOM or ARIA changes), uses Keep a Changelog headings, and no longer claims the semver spec mandates minor-as-breaking below 1.0.
  - **accessibility-per-component** looks for runtime evidence (Storybook test-runner, axe, a transcript) before auditing and says in its first line when there is none; adds severity and evidence columns; drops 2.4.1 (page-level); adds forced-colours, reflow, 4.1.3, 3.3.1 and 3.3.2; and tells the remediation examples to merge the consumer's existing handlers rather than replace them. **codemod-generator** can now run jscodeshift and the test runner it tells the model to run, visits template literals, and scales its test requirement to the codemod type. **cicd-integration** adds the hardcoded-value step (Stylelint), validates DTCG with Terrazzo or a Style Dictionary build rather than a JSON Schema that doesn't exist, and keeps one workflow inline with notes for the other platforms.
  - **docs-coverage** no longer counts an `autodocs` tag as "described": rung 2 needs prop descriptions. **usage-guidelines** and **ai-component-description** read `.ai/metadata/` first and cite a source on every accessibility line; the description is written to Figma only after the user confirms the exact text. **metadata-schema-generator** extracts with docgen tooling, extends the Custom Elements Manifest and docgen shapes instead of inventing a schema, keeps the raw type, and validates with ajv or says it didn't. **pattern-documentation** gains "How it works", a transition table, content guidance, and Evidence and Reference implementation sections that say "none yet" instead of being omitted. **component-decision-tree** writes markdown "choosing between" pages as its output (YAML on request), replaces the Modal/Dialog example (modal is a property of a dialog) and routes an actionable toast to a persistent Alert with the reason. **codebase-index** records each component's token bindings, which is the graph token-audit and component-audit now read. **decision-record** matches the team's existing ADR folder, template and sequential numbering, adds decision drivers and deciders, writes the file, and keeps the "which skill next" advice out of the permanent record. **visual-report** shows statuses as a strip rather than a radar (area implies a magnitude the labels don't have), draws the effort-versus-impact matrix only when the source report carries effort figures, and inlines or pins Chart.js.
  - **adoption-report** gains an engineering-adoption recipe from code (imports, system instances against local look-alikes, token references), stops with a data-collection plan when no team has a measured signal, drops its unsourced thresholds and its platform-reliability and AI-tooling sections, and gives every team row an evidence column. **stakeholder-brief** no longer coaches leaving out figures that "sound small". **system-pitch** no longer supplies conservative-sounding numbers where data is missing, separates year-1 build cost from run cost, adds a sensitivity line, and credits Nathan Curtis's team models in place of its own invented three with FTE ranges. **deprecation-process** names the mechanics (`@deprecated`, `$deprecated`, a once-only dev warning, the lint rules, the Figma rename and Deprecated page), finds consumers in other repositories, and drops the "soft removal" stage that was deprecation under another name. **change-communication** reads the CHANGELOG, git history and changesets first, and adds the design side: Figma library publish notes and the designers' channel. **contribution-workflow** chooses lightweight, standard or full from capacity before writing, writes the proposal template as a GitHub issue form, stops when nobody owns the system, and credits the shape. **backlog-generator** uses the team's own sizing scale, copies the audit's evidence into each card, and exports a tracker CSV on request. **release-retrospective** measures timeline and migration completion from git and the usage search before asking, and gives each recommendation an owner and a date.
  - **Agents:** the diagnostic no longer infers adoption from drift and then reasons from its own guess; the governance review stops for confirmation before writing the outward brief, reports adoption as counts of teams (never percentages), and takes the maturity stage from system-health rather than a config key that never existed; the release pipeline takes every semver call from version-bump-advisor rather than hard-coding "new component = minor".
...
- **The two design-QA wordings that never routed now do.** 1.4.0 left "do a proper design QA before release" and "review the implementation against the handover" as known gaps: Claude explored the repo and did the work by hand. `design-to-code-check`'s description now names design QA, handover review and sign-off as reasons to load it, and both wordings route 3 of 3 (were 0 of 3), with the three "does it match" wordings still routing. `drift-detection` had the same failure once under the richer fixture; its description now leads with when to reach for it and routes 3 of 3.
...
- **Claude often skipped `stakeholder-brief` and `design-to-code-check`** and did the task by hand, losing the skill's structure. Their descriptions listed trigger phrases but never said when to reach for the skill. Measured with routing evals, three runs per wording:
  - `stakeholder-brief` now routes 5 of 6 across two wordings (was 1 of 6).
  - `design-to-code-check` now routes 3 of 3 for "does it match its design/spec" (was 0 of 3). Other phrasings still route 0 of 3 with every description tried, including a broader one that made the common phrasing worse: quick checks with the spec pasted in are usually done without the skill. For the full report, run `/design-system-ops:design-to-code-check` by name. The two unsolved wordings stay in the evals as known gaps.
