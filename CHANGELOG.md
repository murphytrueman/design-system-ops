...
- **Claude sometimes skipped `design-to-code-check` and `stakeholder-brief`** and did the task by hand, losing the skill's structure. Their descriptions listed trigger phrases but never said when to reach for the skill; they now say to use it even for a single component with the spec pasted in, or a short update. Measured with the routing evals before and after.
