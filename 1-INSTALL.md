...
Git clone (above) and the `.plugin` bundle in Cowork are the only supported methods. Both keep the repo layout intact, which matters: skills load their reference material from the shared `knowledge-notes/` directory at the repo root.

Third-party installers that flatten each skill into its own folder — for example `npx skills install` — drop `knowledge-notes/` and `commands/`. Every skill that depends on a knowledge note checks for it before starting, and will stop and tell you the install is incomplete rather than answer without it.

### Verify your install

From a terminal, run the doctor script that ships with the pack:

```bash
bash ~/.claude/skills/design-system-ops/verify-install.sh
```

It checks that every file a skill lists under `references:` exists, prints `OK` or `MISSING` per skill, and ends with `All N references resolve.` on a healthy install. If anything is missing, reinstall with git clone or the `.plugin` bundle. For a project-level install, pass the path: `bash verify-install.sh your-project/.claude/skills/design-system-ops`.
...
The [standard Figma MCP](https://help.figma.com/hc/en-us/articles/32132100833559-Guide-to-the-Figma-MCP-server) from Figma lets Claude read what you have selected in Figma (component data, the variables a selection uses, design specs) and write through its `use_figma` tool. It can't list whole variable collections, so collection-wide audits such as `figma-variable-audit` need the Console MCP.
...
| `sample-outputs/` | Nine unedited skill runs against the test fixture, showing expected depth and format |
...
| `llms.txt` | A short map of the pack for AI agents and indexers |
